...
from DrawTool import draw_day_action_num
model_dict = {
    "RNN": RNN,
    "LSCNN": LSCNN,
}
...
def main():
    ...
    parser.add_argument('--batch_size', type=int, default=4, help='batch_size e.g. 32 64')
    ...
    if params.DataSet == 'Kwai':
        ...
        train_set, valid_set, test_set, day_numpy, param = getDataLoader(params.batch_size, param,data_name='Kwai',data_path='./data/Kwai')
    elif params.DataSet == 'KDD':
        train_set, valid_set, test_set, day_numpy, param = KddGetDataLoader(params.batch_size, param)
    elif params.DataSet == 'Baidu':
        train_set, valid_set, test_set, day_numpy, param = getDataLoader(params.batch_size, param,data_name='Baidu',data_path='./data/Baidu')
    ...
    for i in range(params.max_iter):
        ...
        run(i, train_set, model, optimizer, device, model_name, run_type="train", lossFun=params.LossFun, write_file=f,
            model_param=model_param, fileName=fileName)
        model.eval()
        valid_auc, valid_rmse, valid_df, valid_MAE = run(-1, valid_set, model, optimizer, device, model_name,
                                                         run_type="valid",
        ...
        if (params.LossFun == 'BCE' and valid_auc > best_valid_auc) or (
            ...
            test_auc, test_rmse, test_df, test_MAE = run(-1, test_set, model, optimizer, device, model_name,
                                                         run_type="test",
                                                         write_file=f, model_param=model_param, fileName=fileName)
    ...
    f.write(print_str + "\n")
    f.close()
    if params.seed == 1 and params.model_name == 'FLTADP' and params.multi_task_enable and params.fine_grained:
        draw_day_action_num(fileName + 'pred_1.npy', fileName + '_1.npy', params.DataSet)
