...
def load_data(past_day,future_day=23):
    ...
    df_train_id = df_u['enrollment_id']
    df_test_id = test_df_u['enrollment_id']
    df_train_time = pd.merge(df_train_id, df_train_time, on='enrollment_id', how='left')
    df_test_time = pd.merge(df_test_id,df_test_time,on='enrollment_id',how='left')
...
def getDataLoader(batch_size=32, params={}):
    ...
    draw_day_action_num(y,train_time,past_day,future_day,params["a_feat_size"])
...
def draw_week_action_num(y,train_time,past_day,future_day,a_feat_size):
    #[user_num,a_feat_size*future_day]
    action_y=y[:,2:]
    action_y[action_y != 0] = 1
    action_y=action_y.reshape((-1,future_day,a_feat_size))
    week_time=train_time[:,past_day:,3]
    res = np.zeros((7,a_feat_size))
    r,c=week_time.shape
    ...
    for  i in range(0,r):
        for j in range(0,c):
            week=week_time[i][j]
            week = int(week)
            res[week-1] +=action_y[i][j]
    print(res)
def draw_day_action_num(y,train_time,past_day,future_day,a_feat_size):
    print(a_feat_size)
    #action_y : future_day * feature_num
    action_y = y[:, 2:]
    action_y[action_y != 0] = 1
    action_y = action_y.reshape((-1, future_day, a_feat_size))
    ...
    colors = ['red', 'green', 'blue','yellow','gold', 'cyan', 'm']
    label = 'feature_'
